...
export default function Dashboard() {
    ...
    const handleLocalSync = async (options: any[], residents: any[], barangays: any[]) => {
        ...
        for(const barangay of barangays) {
            await db.runAsync(
                `INSERT INTO barangays ( id, name, code, municipality, province, region, created_at, updated_at )
                VALUES ( ?, ?, ?, ?, ?, ?, ?, ? )
                ON CONFLICT(id) DO UPDATE SET
                    name = excluded.name,
                    code = excluded.code,
                    ...
                    updated_at = excluded.updated_at
                WHERE
                    name != excluded.name OR code != excluded.code OR municipality != excluded.municipality OR province != excluded.provinceOR region != excluded.region OR updated_at != excluded.updated_at`,
                [
                    barangay.id,
                    barangay.name,
                    barangay.code,
