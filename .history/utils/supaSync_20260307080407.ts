import { ResidentData } from '@/app/form';
            ...
            if (state.isConnected && state.isInternetReachable) {
                ...
                for (const residentData of unsynced) {
                    ...
                    if(error) {
                        ...
                        if (cleanedData.school?.length > 0 && cleanedData.osy == 0) {
                            const schools = cleanedData.schools.map((s: any) => ({
                                brgy_resident_id: cleanedData.id,
    ...
    export async function addResident(residentData: ResidentData) {
        ...
        if (error) {
            ...
            if (cleanedData.school?.length > 0 && cleanedData.osy == 0) {
                ...
                const schools = cleanedData.school.map((s: any) => ({
                    brgy_resident_id: id,
                    educational_attainment_school_id: s.educational_attainment_school_id,
                    strand_id: s.strand_id,
                    degree_id: s.degree_id,
