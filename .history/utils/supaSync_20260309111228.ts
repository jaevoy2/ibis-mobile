    ...
    try {
        if (state.isConnected && state.isInternetReachable) {
            ...
            for(const option of unsyncedOptions) {
                const cleanedOption = sanitizeOptions(option);
            ...
            for(const barangay of unsyncedBarangay) {
                ...
                if(error) {
                    ...
                    residents$[barangay.id].assign({ synced: true })
                }
            }
...
export async function addResident(residentData: ResidentData) {
    ...
    if (error) {
        ...
        if(cleanedData.school.length > 0) {
            addEducation(cleanedData.school, cleanedData.id)
        }
...
export async function addEducation(educations: EducationProps[], residentId: string) {
    try {
        for (const education of educations) {
            ...
            const cleanedData = sanitizeEducation(newEducationData);
