...
// update the NetInfo.addEventListener
...
function sanitizeBrgys(brgys: any) {
    const {
        synced,
        ...allowed
    } = brgys;
...
function sanitizeEducation(brgys: any) {
    const {
        synced,
        ...allowed
    } = brgys;
...
export async function addOption(optionData: OptionProps) {
    ...
    const cleanedData = sanitizeOptions(newOptionData);

    await db.runAsync(
        `INSERT INTO options (id, name, category, created_at, updated_at, synced) VALUES (?, ?, ?, ?, ?, ?)`,
        [id, cleanedData.name, cleanedData.category, cleanedData.created_at, cleanedData.updated_at, 0]
    );
...
export async function addBrgys(brgys: Barangays) {
    ...
    const cleanedData = sanitizeBrgys(newBrgysData);

    await db.runAsync(
        `INSERT INTO barangays (id, name, code, municipality, province, region, created_at, updated_at, synced) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [ id, cleanedData.name, cleanedData.code, cleanedData.municipality, cleanedData.province, cleanedData.region, cleanedData.created_at, cleanedData.updated_at, 0 ]
    );
...
export async function addEducation(educations: EducationProps[], residentId: string) {
    for (const education of educations) {
        ...
        const newEducationData = {
            ...education,
            id,
            brgy_resident_id: residentId,
            created_at: new Date().toISOString(),
            updated_at: new Date().toISOString(),
        };

        const cleanedData = sanitizeResidentForInsert(education);

        residents$[id].assign(cleanedData);

        try {
            await db.runAsync(
                `INSERT INTO resident_school 
                (id, brgy_resident_id, educational_attainment_school_id, strand_id, degree_id, school, year_graduated, created_at, updated_at, synced) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                [
                    newEducationData.id,
                    newEducationData.brgy_resident_id,
                    newEducationData.educational_attainment_school_id ?? null,
                    newEducationData.strand_id ?? null,
                    newEducationData.degree_id ?? null,
                    newEducationData.school ?? null,
                    newEducationData.year_graduated ?? null,
                    newEducationData.created_at,
                    newEducationData.updated_at,
                    0,
                ]
            );
            ...
            if (error) {
                ...
                await db.runAsync(
                    `UPDATE resident_school SET synced = ? WHERE id = ?`,
                    [1, id]
                );
            }
        } catch (err) {
