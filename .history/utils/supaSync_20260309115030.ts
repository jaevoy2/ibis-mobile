...
NetInfo.addEventListener(async (state) => {
    try {
        if (state.isConnected && state.isInternetReachable) {
            const unsyncedOptions = Object.values(optionProps$.get()).filter(opt => opt.synced == 0);
            for(const option of unsyncedOptions) {
                const cleanedOption = sanitizeData(option);
                const { error } = await supabase.from('options').insert([cleanedOption]).select();
                if(error) {
                    console.log(`Supabase insert failed for ${option.id}:`, error);
                }else {
                    optionProps$[option.id].assign({ synced: 1 })
                }
            }

            const unsyncedBarangay = Object.values(barangay$.get()).filter(brgy => brgy.synced == 0);
            for(const barangay of unsyncedBarangay) {
                const cleanedBrgy = sanitizeData(barangay);
                const { error } = await supabase.from('barangays').insert([cleanedBrgy]).select();
                if(error) {
                    console.log(`Supabase insert failed for ${barangay.id}:`, error);
                }else {
                    barangay$[barangay.id].assign({ synced: 1 })
                }
            }
            ...
            const unsyncedEducation = Object.values(education$.get()).filter(edu => edu.synced == 0);
            for (const education of unsyncedEducation) {
                const cleanedEducation = sanitizeData(education);
                const { error } = await supabase.from('educations').insert([cleanedEducation]).select();
                if(error) {
                    console.log(`Supabase insert failed for ${education.id}:`, error);
                }else {
                    education$[education.id].assign({ synced: 1 })
                }
            }

            if(
                unsyncedOptions.length ||
                unsyncedBarangay.length ||
                unsyncedResidents.length ||
                unsyncedEducation.length
            ){
