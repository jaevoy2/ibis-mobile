    ...
    try {
        if (state.isConnected && state.isInternetReachable) {
            ...
            for (const resident of unsynced) {
                ...
                const { error } = await supabase.from('brgy_residents').insert([residentData]).select();
...
export async function addResident(residentData: any) {
    ...
    try {
      const { error } = await supabase.from('brgy_residents').insert([newResidentData]).select();
