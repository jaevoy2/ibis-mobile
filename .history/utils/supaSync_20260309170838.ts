...
NetInfo.addEventListener(async (state) => {
    try {
        if (state.isConnected && state.isInternetReachable) {
            try {
                // Only sync if the stores exist
                if (barangay$) await syncCollection(barangay$, 'barangays');
                if (optionProps$) await syncCollection(optionProps$, 'options');

                const residentsState = residents$?.get?.() ?? {};
                const unsyncedResidents = Object.values(residentsState).filter(r => !r.synced);
                ...
                if (education$) await syncCollection(education$, 'educations');
