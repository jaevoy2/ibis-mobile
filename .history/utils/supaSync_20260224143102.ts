...
NetInfo.addEventListener(async (state) => {
    try {
        if (state.isConnected && state.isInternetReachable) {
            const unsynced = Object.values(residents$).filter(r => !r.get().synced).map(r => r.get());
            if (unsynced.length === 0) return;

            const backendResponse = await handleSyncBackend(unsynced);
            ...
            const { error } = await supabase.from('brgys_residents').insert(unsynced).select();

            if(error) {
                console.log(`Supabase insert failed: `, error);
            }else {
                unsynced.forEach(resident => {
                    residents$[resident.id].assign({ synced: true });
                });
                Alert.alert(
                    'Sync Complete',
                    'All resident records have been successfully synced.',
                    [
                        {
                        text: 'OK',
                        onPress: () => console.log('User acknowledged sync success'),
                        },
                    ],
                    { cancelable: false }
                );
...
export async function addResident(residentData: any) {
    ...
    try {
      const { error } = await supabase.from('brgys_residents').insert([newResidentData]).select();

      if (error) {
        console.log(`Supabase insert failed for ${id}:`, error);
      } else {
        handleSyncBackend([newResidentData])
