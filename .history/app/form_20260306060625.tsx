...
import { ActivityIndicator, Dimensions, KeyboardAvoidingView, Platform, Pressable, ScrollView, Switch, Text, TextInput, TouchableOpacity, View } from 'react-native';
...
type FamilyProps = { resident_id: number; 
  id: number
  family_name: string; 
  familty_type_id: number; 
  income_level_id: number; 
...
export default function ResidentForm() {
  ...
  const handleAssigning = async () => {
    try {
      ...
      const familiesData: any = await db.getAllAsync(`SELECT * FROM families`);
          ...
          <ScrollView style={{ padding: 16 }}>
            ...
            <View style={{ borderColor: '#C6C6C6', borderWidth: 1, borderRadius: 10, marginBottom: 15 }}>
              ...
              <View style={{ padding: 10 }}>
                ...
                <View style={{ borderColor: '#C6C6C6', borderWidth: 1, borderRadius: 10, height: 50, justifyContent: 'center' }}>
                  ...
                  >
                    {options.filter(d => d.category == 'relationship_contact').map(r => <Picker.Item key={r.id} label={r.name} value={r.id} />)}
                  </Picker>
                </View>
              </View>
            </View>
                ...
                {votingRegistration.map(({ label, key, value }, index) => (
                  <View key={index} style={{ width: '100%', marginBottom: label != 'Voting Eligibility' ? 12 : 0 }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                      <Switch
                      disabled={label == 'Voting Eligibility'}
                        value={!!residentData?.[key]}
                        onValueChange={(val) => handleOnChange(key, val)}
                      />
                      <Text style={{ flex: 1, fontSize: 12 }}>{label}</Text>
                    ...
                    {label == 'Voting Eligibility' && (
                      <Text style={{ fontWeight: '400', fontSize: 12, marginTop: -10, marginBottom: 8, marginLeft: 47,color: '#8f8f8f' }}>Automatically set to true when age is 15 or older

    </Text>
                    )}
                    {label == 'Registered Voter' && residentData?.[key] == true && (
                      <TextInput
                        placeholder={`Enter Registered Barangay`}
                        onChangeText={(text) => handleOnChange(value, text)}
                        style={{ borderWidth: 1, borderColor: '#ccc', padding: 10, borderRadius: 4 }}
                      />
                    )}
            ...
            <View style={{ borderColor: '#C6C6C6', borderWidth: 1, borderRadius: 10, marginBottom: 20 }}>
              ...
              <View style={{ padding: 10, flexDirection: 'column' }}>
                  ...
                  <View style={{ borderColor: '#C6C6C6', borderWidth: 1, borderRadius: 10, height: 50, justifyContent: 'center' }}>
                    ...
                    >
                      {['Active', 'Inactive', 'Deceased'].map((s) => (
                        <Picker.Item key={s} label={s} value={s} style={{ fontSize: 13 }} />
                      ))}
                    </Picker>
                  </View>
                </View>
                ...
                <View style={{ width: '100%', flexDirection: 'row', alignItems: 'center', paddingRight: 8 }}>
                  <Switch
                    onValueChange={(val) => handleOnChange('inactive_type', val)}
                  />
                  <Text style={{ flex: 1, fontSize: 12 }}>Inactive</Text>
                </View>
              </View>
            </View>
                ...
                <View style={{ borderWidth: 1, borderColor: '#C6C6C6', borderRadius: 10, marginBottom: 10, height: 50, justifyContent: 'center' }}>
                  ...
                  >
                    ...
                    {data.filter(d => d.category == 'educational_attainment_school').map((att) => (
                      <Picker.Item key={att.id} label={att.name} value={att.id} style={{ fontSize: 13 }} />
                    ))}
                  </Picker>
                </View>
            ...
            <Pressable style={{ flexDirection: 'row', alignItems: 'center', gap: 5, padding: 10, backgroundColor: '#306060', alignSelf: 'center', marginBottom: 15, borderRadius: 8 }}>
