      ...
      <ScrollView style={{ padding: 16 }}>
          ...
          <View style={{ flexDirection: 'row' }}>
              <View style={{ flexDirection: 'column', width: '50%' }}>
                {specialStatus.map((item, index) => (
                  <View key={index} style={{ width: '10%', flexDirection: 'row', alignItems: 'center', marginBottom: 12, paddingRight: 8 }}>
                    <Switch
              ...
              <View style={{ flexDirection: 'column', width: '50%' }}>
                {specialStatusCol2.map((item, index) => (
                  <View key={index} style={{ width: '100%', flexDirection: 'row', alignItems: 'center', marginBottom: 12, paddingRight: 8 }}>
                    <Switch
                      value={!!switchState[item]}
                      onValueChange={(val) => handleSwitchChange(item, val)}
                    />
        ...
        <View style={{ borderColor: '#C6C6C6', borderWidth: 1, borderRadius: 10, marginBottom: 20 }}>
          ...
          <View style={{ padding: 10, flexDirection: 'column' }}>
            ...
            <View style={{ width: '100%', marginBottom: 12 }}>
              ...
              <View style={{ borderColor: '#C6C6C6', borderWidth: 1, borderRadius: 10 }}>
                <Picker
                  ...
                  onValueChange={(val) => handleChange('status_id', val)}
                >
            ...
            <View style={{ marginBottom: 12 }}>
              ...
              <TouchableOpacity
                onPress={() => setShowPicker('date_of_death')}
                style={{
                  flexDirection: 'row',
                  justifyContent: 'space-between',
                  paddingHorizontal: 10,
              ...
              >
                <Text>{form.date_of_death ? (form.date_of_death as Date).toDateString() : 'Select date'}</Text>
                <Ionicons name="calendar" size={20} color="#989898" />
              </TouchableOpacity>
              {showPicker === 'date_of_death' && (
                <DateTimePicker
                  value={form.date_of_death as Date || new Date()}
                  mode="date"
                  display="default"
                  onChange={(_, date) => {
                    if (date) handleChange('date_of_death', date);
                    setShowPicker(null);
                  }}
                />
              )}
            ...
            <View style={{ marginBottom: 12 }}>
              ...
              <TouchableOpacity
                onPress={() => setShowPicker('date_of_migration')}
                style={{
                  flexDirection: 'row',
                  justifyContent: 'space-between',
                  paddingHorizontal: 10,
              ...
              >
                <Text>{form.date_of_migration ? (form.date_of_migration as Date).toDateString() : 'Select date'}</Text>
                <Ionicons name="calendar" size={20} color="#989898" />
              </TouchableOpacity>
              {showPicker === 'date_of_migration' && (
                <DateTimePicker
                  value={form.date_of_migration as Date || new Date()}
                  mode="date"
                  display="default"
                  onChange={(_, date) => {
                    if (date) handleChange('date_of_migration', date);
                    setShowPicker(null);
                  }}
                />
              )}
            ...
            <View style={{ width: '100%', marginBottom: 12 }}>
              ...
              <TextInput
                value={form.place_to_migrate}
                onChangeText={(text) => handleChange('place_to_migrate', text)}
        ...
        <View style={{ borderColor: '#C6C6C6', borderWidth: 1, borderRadius: 10, marginBottom: 20 }}>
          ...
          <View style={{ padding: 10 }}>
            ...
            <View style={{ borderWidth: 1, borderColor: '#C6C6C6', borderRadius: 10, marginBottom: 10 }}>
              <Picker
                mode='dropdown'
                selectedValue={form.educational_attainment_school_id}
                onValueChange={(val) => handleChange('educational_attainment_school_id', val)}
              >
            ...
            <TextInput
              ...
              onChangeText={(text) => handleChange('year_graduated', text)}
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 10 }}>
              <Switch
                value={form.osy}
                onValueChange={(val) => handleChange('osy', val)}
              />
        ...
        <View style={{ marginBottom: 12 }}>
          ...
          <View style={{ borderWidth: 1, borderColor: '#C6C6C6', borderRadius: 10 }}>
            <Picker
              mode="dropdown"
              selectedValue={form.family_id}
              onValueChange={(val) => handleChange('family_id', val)}
            >
