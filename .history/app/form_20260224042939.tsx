  ...
  return (
    <View style={{ paddingBottom: 50, height }}>
      ...
      <ScrollView style={{ padding: 16 }}>
        ...
        {/* Boolean switches in 2 columns */}
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginBottom: 12 }}>
          {([
            'voting_eligibility','registered_voter','osy','is_4ps_member','pwd','senior',
            'indigent_senior','pensioner','ofw','indigenous_people','solo_parent',
            'has_ph','has_sss','has_gsis','has_hdmf'
          ] as (keyof typeof form)[]).map((field, index) => (
            <View key={field} style={{ width: '50%', flexDirection: 'row', alignItems: 'center', marginBottom: 12, paddingRight: 8 }}>
              <Text style={{ flex: 1, fontSize: 12 }}>{field.replace(/_/g,' ').toUpperCase()}</Text>
              <Switch
                value={!!form[field]}
        ...
        {(['date_of_birth','date_of_death','date_of_migration'] as (keyof typeof form)[]).map(field => (
          <View key={field} style={{ marginBottom: 12 }}>
            ...
            <TouchableOpacity
              ...
              style={{ backgroundColor: '#ebebebff', paddingVertical: 8, borderRadius: 6, marginTop: 4 }}
            >
              <Text style={{ color: '#000', textAlign: 'center' }}>
                {form[field] ? (form[field] as Date).toDateString() : 'Select date'}
              </Text>
            </TouchableOpacity>
            {showPicker === field && (
              <DateTimePicker
